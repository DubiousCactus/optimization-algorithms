...
\begin{equation}
  g(X)=W^TX+\omega_{0}
\end{equation}
with $W$ being the weights vector, and $\omega_{0}$ the bias, together constituting the discriminant. When the result of this function is above 0, the given $X$ vector can be classified as belonging to the perceptron's class, otherwise it doesn't.
...
\begin{equation}
  \mathcal{J}_p(W)=\underset{x_{i}\in\chi}{\sum}-l_{i}\overset{~}{W}^TX_{i}
\end{equation}
with $\chi$ being the set of misclassified samples for the perceptron, $l_i$ being the binary label of the misclassified sample, $\overset{~}{W}$ corresponding to the augmented weights vector, and $\overset{~}{X_{i}}$ the misclassidied training sample. This optimization problem can be solved using the gradient descent method as follow:
\begin{equation}
  W(t+1)=W(t)+\eta\underset{x_{i}\in\chi}{\sum}-l_{i}X_{i}
\end{equation}
