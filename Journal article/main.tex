...
\begin{equation}
  g(\mathbf{w})=\mathbf{w}^T\mathbf{x}+\omega_{0}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:perceptron}
\end{equation}
with $\chi$ being the set of misclassified samples for the perceptron, $l_i$ being the binary label of the misclassified sample, $\tilde{\mathbf{w}}$ corresponding to the augmented weights vector, and $\tilde{\mathbf{x}}_i$ the misclassidied training sample. This optimization problem can be solved using the gradient descent method as follow:
\begin{equation}
...
\begin{equation}
  \mathcal{J}_p(\mathbf{w}) = ||\mathbf{X}^T\mathbf{w}-\mathbf{b}||_2 ^2
\end{equation}
