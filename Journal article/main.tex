...
\tab The bar diagrams shown in figure 3 and 4 clearly demonstrate that most of the algorithms are more efficient and give slightly better results on the ORL dataset. It is also clear that the distance-based algorithms perform better than the perceptrons, and it is especially true for the Nearest Neighbour classifier, even though it is the slowest algorithm as shown in the following bar diagrams.

INSERT MNIST TIME CONSUMPTION
\\

INSERT ORL TIME CONSUMPTION
\\

\tab As it can be seen on figure 5 and 6, the execution time of the Nearest Neighbour is the highest, as stated above, while the other distance-based classifiers globally remain the fastest to compute.
